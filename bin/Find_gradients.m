...
% Xiaoyan, 2020
function SPOTS = Find_gradients(SPOTS)
...
bandwid=SPOTS.Find_gradients_bandwidth;
% kde
...
XSPOTS=[(1+SPOTS.distance):SPOTS.Find_gradients_distance:(size(density,2)-SPOTS.distance)];
YSPOTS=[(1+SPOTS.distance):SPOTS.Find_gradients_distance:(size(density,1)-SPOTS.distance)];
...
for YS=YSPOTS
    XREAL=[XREAL,XS];
    YREAL=[YREAL,YS];
    VSPOTS=[VSPOTS,(density(YS-SPOTS.Find_gradients_distance,XS)- density(YS+SPOTS.Find_gradients_distance,XS))];
    USPOTS=[USPOTS,(density(YS,XS-SPOTS.Find_gradients_distance)- density(YS,XS+SPOTS.Find_gradients_distance))];
end
end
% figure
