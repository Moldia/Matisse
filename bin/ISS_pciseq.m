function CELLS = ISS_pciseq(SPOTS,CellMap,gSet)
% o = o.call_cells(gSet)
%  
...
IncludeSpot = inpolygon(o.SpotGlobalYX(:,2), o.SpotGlobalYX(:,1), SPOTS.CellCallRegionYX(:,2), SPOTS.CellCallRegionYX(:,1));
IncludeSpot = ~ismember(AllGeneNames, ExcludeGenes) ...
    & inpolygon(o.SpotGlobalYX(:,1), o.SpotGlobalYX(:,2), SPOTS.CellCallRegionYX(:,2), SPOTS.CellCallRegionYX(:,1));%...
    %& o.SpotScore> quality;
...
y0 = min(SPOTS.CellCallRegionYX(:,1));
x0 = min(SPOTS.CellCallRegionYX(:,2));
y1 = max(SPOTS.CellCallRegionYX(:,1));
x1 = max(SPOTS.CellCallRegionYX(:,2));
...
CELLS.matrixcount=matrixcount;
CELLS.geneames=GeneNames;
CELLS.name=CELLCLASS(1:end-1)';
CELLS.pos=[CellYX(:,2),CellYX(:,1)];
CELLS.ClassNames=ClassNames;
CELLS.Probabilities=pCellClass;
...
% csvwrite('pCellClass_testCA1.csv',o.pCellClass);
% T=cell2table(o.ClassNames);
% writetable(T,'ClassNames_testCA1.csv');
% csvwrite('CellYX_testCA1.csv',o.CellYX);
